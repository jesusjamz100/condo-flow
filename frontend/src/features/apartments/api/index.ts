...
// Admin
export async function getAllApartments(page: number = 0, size: number = 10) {
    const { data } = await httpClient.get<PageResponse<ApartmentResponse>>(`/apartments/admin?page=${page}&size=${size}`)
    return data;
}
